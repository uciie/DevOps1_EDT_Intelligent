...
public class CalendarImportController {
     ...
     */
    @PostMapping
    public ResponseEntity<String> importCalendar(@RequestParam("file") MultipartFile file, User u) {
        try {
            List<Event> le = importService.importCalendar(file,u);
            return ResponseEntity.ok(le.size() + " événements importés avec succès.");
        } catch (Exception e) {
